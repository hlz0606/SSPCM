...
config.MIXUP_mode = False
config.different_domain_data_mixup = False
config.label_data_mixup = False
config.unlabel_data_mixup = False
config.mixup_beta = 0.8

config.ensemble_mode = 'mode1'
config.flip_ensemble = False

config.score_filter = False
config.two_merge_model_score_filter = False
config.score_filter_thread = 0.1
config.score_diff_reweight = False


config.kp_pixel_dist_filter = False
config.two_merge_model_kp_dist_filter = False
config.kp_pixel_dist_filter_thread = 150  
config.kp_pixel_dist_diff_reweight = False


config.kp_dist_and_score_both_satisfy = False   

config.keypoint_perception_cutmix_num = 0  



config.feature_attack_train = False
config.feature_attack_mask_num = 10

config.uncertainty_and_time_correction = False
config.time_correction_warmup_epoch = 0

config.use_diff_model = False
...
config.OTHER_MODEL = edict()
config.OTHER_MODEL.MODEL = edict()
# Only Used for SSL
config.OTHER_MODEL.MODEL.BACKBONE = 'resnet'
config.OTHER_MODEL.MODEL.PRM = False

config.OTHER_MODEL.MODEL.NAME = 'pose_resnet'
config.OTHER_MODEL.MODEL.INIT_WEIGHTS = True
config.OTHER_MODEL.MODEL.PRETRAINED = ''
config.OTHER_MODEL.MODEL.TCH_PRETRAINED = ''
config.OTHER_MODEL.MODEL.SEG_PRETRAINED = ''
config.OTHER_MODEL.MODEL.NUM_JOINTS = 16
config.OTHER_MODEL.MODEL.IMAGE_SIZE = [256, 256]  # width * height, ex: 192 * 256
config.OTHER_MODEL.MODEL.GROUPS = 1
config.OTHER_MODEL.MODEL.WIDTH_PER_GROUP = 64
config.OTHER_MODEL.MODEL.EXTRA = MODEL_EXTRAS[config.OTHER_MODEL.MODEL.NAME]
...
def get_model_name(cfg):
    ...
    if name in ['pose_resnet', 'pose_dual', 'pose_cons', 'pose_hrnet',
                'my_pose_cons', 'my_pose_dual', 'my_pose_triple', 'my_pose_ensemble']:
