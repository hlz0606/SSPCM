...
import sys
sys.path.append('/aistudio/workspace/external-libraries')
...
from core.loss import JointsMSELoss, PoseCoLoss, PoseDisLoss, PoseTripleLoss
...
def main():
    ...
    last_epoch_model = eval('models.'+config.MODEL.NAME+'.get_pose_net')(
        config, is_train=True
    )
    ...
    if args.distributed:
        ...
        last_epoch_model = last_epoch_model.to(device)
        last_epoch_model = torch.nn.parallel.DistributedDataParallel(last_epoch_model,device_ids=[local_rank],
                        output_device=local_rank, find_unused_parameters=True)
    else:
        model = torch.nn.DataParallel(model, device_ids=gpus).cuda()
        last_epoch_model = torch.nn.DataParallel(last_epoch_model, device_ids=gpus).cuda()

    if config.MODEL.NAME in ['pose_resnet', 'pose_hrnet']:
        criterion = JointsMSELoss(
            use_target_weight=config.LOSS.USE_TARGET_WEIGHT, cfg = config,
        ).cuda()
    elif config.MODEL.NAME in ['pose_dual', 'my_pose_dual', 'my_pose_ensemble']:
        criterion = PoseCoLoss(
            use_target_weight=config.LOSS.USE_TARGET_WEIGHT, cfg = config,
        ).cuda()
    elif config.MODEL.NAME in ['pose_cons', 'my_pose_cons']:
        criterion = PoseDisLoss(
            use_target_weight=config.LOSS.USE_TARGET_WEIGHT, cfg = config,
        ).cuda()
    elif config.MODEL.NAME in ['my_pose_triple']:
        criterion = PoseTripleLoss(
            use_target_weight=config.LOSS.USE_TARGET_WEIGHT, cfg = config,
        ).cuda()
    ...
    ######
    filename = os.path.join(final_output_dir, 'last_epoch_model.pth.tar')
    logger.info('=> saving last_epoch_model to {}'.format(filename))
    torch.save(model.module.state_dict(), filename)
    last_epoch_model.module.load_state_dict(torch.load(filename))
